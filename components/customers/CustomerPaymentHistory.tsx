...
export default function CustomerPaymentHistory({
  ...
  async function fetchPage(p: number) {
    ...
    try {
      ...
      const totalFromApi =
        typeof j?.total === "number" ? j.total : list.length
      ...
      <CardHeader className="pb-3">
        <div className="flex items-start justify-between">
          <div>
            ...
            <CardDescription>
              Εμφανίζονται κινήσεις για τον επιλεγμένο πελάτη.
            </CardDescription>
          </div>
          ...
          <div className="overflow-x-auto border rounded-md">
            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              <thead className="bg-gray-50 dark:bg-gray-800">
                <tr>
                  <th className="px-6 py-2 text-left text-xs font-medium uppercase tracking-wider">Ημ/νία</th>
                  <th className="px-6 py-2 text-left text-xs font-medium uppercase tracking-wider">Ποσό</th>
                  <th className="px-6 py-2 text-left text-xs font-medium uppercase tracking-wider">Τύπος</th>
                  <th className="px-6 py-2 text-left text-xs font-medium uppercase tracking-wider">Σημειώσεις</th>
                </tr>
              </thead>
...
function renderTypeBadge(type?: string) {
  ...
  if (t === "debt") {
    return <Badge className="bg-yellow-100 text-yellow-800 dark:bg-yellow-800 dark:text-yellow-100">Οφειλή</Badge>
  }
  return <Badge className="bg-green-100 text-green-800 dark:bg-green-800 dark:text-green-100">Είσπραξη</Badge>
}
