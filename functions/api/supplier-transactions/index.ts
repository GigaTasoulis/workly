...
export async function onRequest({ request, env }: any) {
  ...
  if (method === "POST") {
    ...
    const owns = await env.DB.prepare(`SELECT 1 FROM suppliers WHERE user_id = ? AND id = ?`)
      .bind(user.user_id, supplier_id)
      .first();
    ...
    await env.DB.prepare(
      ...
      .bind(id, user.user_id, supplier_id, product_name, amount, amount_paid, date, status, notes)
      .run();
