export async function onRequest({ request, env }: any) {
  ...
  if (method === "GET") {
    ...
    if (employeeId) { where.push("employee_id = ?"); vals.push(employeeId); }
    if (from) { where.push("date >= ?"); vals.push(from); }
    if (to)   { where.push("date <= ?"); vals.push(to); }
    if (status) {
      if (status !== "paid" && status !== "pending") return json({ error: "Invalid status" }, 400, cors());
  ...
  if (method === "POST") {
    ...
    if (!Number.isFinite(hours_worked) || hours_worked < 0) return json({ error: "hours_worked must be >= 0" }, 400, cors());
    if (!Number.isFinite(total_amount) || total_amount < 0) return json({ error: "total_amount must be >= 0" }, 400, cors());
    if (!Number.isFinite(amount_paid) || amount_paid < 0) return json({ error: "amount_paid must be >= 0" }, 400, cors());

    const emp = await env.DB.prepare(
      `SELECT 1 FROM employees WHERE user_id = ? AND id = ?`
    ).bind(user.user_id, employee_id).first();
    ...
    await env.DB.prepare(
      `INSERT INTO worklogs
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`
    ).bind(id, user.user_id, employee_id, workplace_id, date, hours_worked, notes, total_amount, amount_paid).run();

    return json({ worklog: { id, employee_id, workplace_id, date, hours_worked, notes, total_amount, amount_paid } }, 201, cors());
  }
