export async function onRequest({ request, env }: any) {
  ...
  if (method === "POST") {
    ...
    const status = (String(b?.status ?? "pending").trim() || "pending") as
      | "paid"
      | "pending"
      | "cancelled";
    ...
    const owns = await env.DB.prepare(`SELECT 1 FROM customers WHERE id = ? AND user_id = ?`)
      .bind(customer_id, user.user_id)
      .first();
    ...
    await env.DB.prepare(
      ...
      .bind(id, user.user_id, customer_id, product_name, amount, amount_paid, date, status, notes)
      .run();
