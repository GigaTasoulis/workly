export async function onRequest({ request, env, params }: any) {
    ...
    if (method === "PUT") {
      const exists = await env.DB.prepare(
        `SELECT 1 FROM customers WHERE user_id = ? AND id = ?`
      ).bind(user.user_id, id).first();
      ...
      if (b?.contact_person !== undefined || b?.contactPerson !== undefined) {
        sets.push("contact_person = ?"); vals.push((b.contact_person ?? b.contactPerson ?? "").toString().trim() || null);
      }
      if (b?.email !== undefined)   { sets.push("email = ?");   vals.push((b.email ?? "").toString().trim() || null); }
      if (b?.phone !== undefined)   { sets.push("phone = ?");   vals.push((b.phone ?? "").toString().trim() || null); }
      if (b?.address !== undefined) { sets.push("address = ?"); vals.push((b.address ?? "").toString().trim() || null); }
      if (b?.afm !== undefined)     { sets.push("afm = ?");     vals.push((b.afm ?? "").toString().trim() || null); }
      if (b?.tractor !== undefined) { sets.push("tractor = ?"); vals.push((b.tractor ?? "").toString().trim() || null); }
    ...
    if (method === "DELETE") {
      const res = await env.DB.prepare(
        `DELETE FROM customers WHERE user_id = ? AND id = ?`
