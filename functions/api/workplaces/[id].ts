...
export async function onRequest({ request, env, params }: any) {
  ...
  if (method === "PUT") {
    ...
    const existing = await env.DB.prepare(`SELECT id FROM workplaces WHERE user_id = ? AND id = ?`)
      .bind(user.user_id, id)
      .first();
    ...
    if (body?.address !== undefined) {
      sets.push("address = ?");
      vals.push(String(body.address).trim());
    }
    if (body?.city !== undefined) {
      sets.push("city = ?");
      vals.push(String(body.city).trim());
    }
    if (body?.state !== undefined) {
      sets.push("state = ?");
      vals.push(String(body.state).trim());
    }
    ...
    if (body?.capacity !== undefined) {
      sets.push("capacity = ?");
      vals.push(String(body.capacity).trim());
    }
    if (body?.notes !== undefined) {
      sets.push("notes = ?");
      vals.push(String(body.notes).toString().trim().slice(0, 2000));
    }
  ...
  if (method === "DELETE") {
    const res = await env.DB.prepare(`DELETE FROM workplaces WHERE user_id = ? AND id = ?`)
      .bind(user.user_id, id)
      .run();
