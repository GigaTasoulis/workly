...
export default function SupplierPaymentHistory({
  ...
  const filtered = useMemo(() => {
    ...
    if (term) {
      list = list.filter((r) =>
        [r.supplierName || supplierNameById[r.supplierId] || "Unknown", r.productName || "", r.notes || ""]
          .join(" ")
          .toLowerCase()
          .includes(term)
      )
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          ...
          <tbody className="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
            {loading ? (
              <tr>
                <td className="px-6 py-4 text-sm" colSpan={7}>Φόρτωση…</td>
              </tr>
            ) : pageRows.length === 0 ? (
              <tr>
                <td className="px-6 py-4 text-sm" colSpan={7}>Δεν υπάρχουν εγγραφές.</td>
              </tr>
            ) : (
              pageRows.map((r) => {
                ...
                return (
                  <tr key={r.id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm dark:text-gray-50">
                      {kind}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm dark:text-gray-50">
                      {r.productName || "—"}
