...
export async function onRequest({ request, env, params }: any) {
  ...
  const tx = await env.DB.prepare(
    ...
    .bind(txId, user.user_id, supplierId)
    .first();
  ...
  if (type === "payment") {
    ...
    await env.DB.prepare(
      ...
      .bind(newPaid, newStatus, tx.id)
      .run();

    // Delete the payment row
    await env.DB.prepare(`DELETE FROM supplier_payments WHERE id = ? AND user_id = ?`)
      .bind(id, user.user_id)
      .run();
  ...
  if (type === "debt") {
    ...
    if (Number(tx.amount_paid) > 0) {
      return json(
        { error: "Cannot delete debt: payments already applied. Reverse those payments first." },
        409,
        cors(),
      );
    }
    ...
    await env.DB.prepare(`DELETE FROM supplier_transactions WHERE id = ?`).bind(tx.id).run();
