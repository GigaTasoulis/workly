export async function onRequest({ request, env, params }: any) {
  ...
  if (method === "PUT") {
    const existing = await env.DB.prepare(`SELECT id FROM employees WHERE user_id = ? AND id = ?`)
      .bind(user.user_id, id)
      .first();
    ...
    if (b?.position !== undefined) {
      sets.push("position = ?");
      vals.push(String(b.position).trim());
    }
    if (b?.department !== undefined) {
      sets.push("department = ?");
      vals.push(String(b.department).trim());
    }
    ...
    if (b?.workplace_id !== undefined || b?.workplaceId !== undefined) {
      ...
      if (wp) {
        const owns = await env.DB.prepare(`SELECT 1 FROM workplaces WHERE user_id = ? AND id = ?`)
          .bind(user.user_id, wp)
          .first();
        if (!owns) return json({ error: "workplace not found" }, 404, cors());
        sets.push("workplace_id = ?");
        vals.push(wp);
      } else {
        sets.push("workplace_id = ?");
        vals.push(null);
    ...
    if (b?.notes !== undefined) {
      sets.push("notes = ?");
      vals.push(String(b.notes).toString().trim().slice(0, 2000));
    }
  ...
  if (method === "DELETE") {
    const res = await env.DB.prepare(`DELETE FROM employees WHERE user_id = ? AND id = ?`)
      .bind(user.user_id, id)
      .run();
    if (!res.meta || res.meta.changes === 0) return json({ error: "Not found" }, 404, cors());
    return json({ ok: true }, 200, cors());
