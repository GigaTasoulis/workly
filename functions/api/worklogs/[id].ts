export async function onRequest({ request, env, params }: any) {
  ...
  if (method === "PUT") {
    const existing = await env.DB.prepare(`SELECT id FROM worklogs WHERE user_id = ? AND id = ?`)
      .bind(user.user_id, id)
      .first();
  ...
  if (method === "DELETE") {
    const res = await env.DB.prepare(`DELETE FROM worklogs WHERE user_id = ? AND id = ?`)
      .bind(user.user_id, id)
      .run();
    if (!res.meta || res.meta.changes === 0) return json({ error: "Not found" }, 404, cors());
    return json({ ok: true }, 200, cors());
