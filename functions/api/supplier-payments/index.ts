...
export async function onRequest({ request, env }: any) {
  ...
  if (!hasPayments) {
    return json(
      { error: "Supplier payments table missing. Add a migration for `supplier_payments`." },
      500,
      cors(),
    );
  }
  ...
  if (method === "POST") {
    ...
    if (!["payment", "debt"].includes(type))
      return json({ error: "type must be 'payment' or 'debt'" }, 400, cors());
    ...
    const tx = await env.DB.prepare(
      ...
      .bind(transaction_id, user.user_id)
      .first();

    if (!tx) return json({ error: "Not found: transaction" }, 404, cors());
    if (tx.supplier_id !== supplier_id)
      return json({ error: "Transaction does not belong to supplier" }, 400, cors());
    ...
    await env.DB.prepare(
      ...
      .bind(id, user.user_id, supplier_id, transaction_id, amount, date, notes, type)
      .run();
    ...
    if (type === "payment") {
      await env.DB.prepare(
        ...
        .bind(amount, amount, transaction_id, user.user_id)
        .run();
    }
