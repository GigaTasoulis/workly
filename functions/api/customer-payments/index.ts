export async function onRequest({ request, env }: any) {
  ...
  if (method === "GET") {
    ...
    const rows = await env.DB.prepare(
      ...
      .bind(user.user_id, customer_id, limit, offset)
      .all();
  ...
  if (method === "POST") {
    ...
    if (tx.status === "cancelled")
      return json({ error: "Cannot pay cancelled transaction" }, 400, cors());
    ...
    await env.DB.prepare(
      ...
      .bind(newPaid, newStatus, transaction_id, user.user_id)
      .run();
